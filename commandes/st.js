...
async function handleSettingCommand(commandeOptions, settingName, settingKey, responses = {}) {
  ...
  if (!superUser) {
    return repondre(`*This command is restricted to the bot owner or ⦍ᴋᴇʟᴠɪɴ ᴊᴀᴍᴘᴀɴ⦐ owner.*`);
  }
...
etatCommands.forEach(cmd => {
  zokou({
    ...
    if (!superUser) {
      return repondre("*This command is restricted to the bot owner or ⦍ᴋᴇʟᴠɪɴ ᴊᴀᴍᴘᴀɴ⦐.*");
    }
